...
function transformIds<T>(
  ...
  if (Array.isArray(data)) {
    ...
    if (data.every((item) => typeof item === "number")) {
      return data.map((num) => s(c).encode([num])) as T;
    }
    return data.map((item) =>
      transformIds(c, item, ignoreKeys, forceTransform)
    ) as T;
  }
...
export const Response = {
  ...
  schema: (
    schema: z.ZodType,
    config: Partial<OpenAPIResponseConfig> = {}
  ): OpenAPIResponseConfig => ({
